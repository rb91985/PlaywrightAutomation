        ...
        - list [ref=f3e123]:
          - listitem [ref=f3e124]:
            - generic [ref=f3e125] [cursor=pointer]:
              ...
              - paragraph [ref=f3e127]: test user
              - generic [ref=f3e128]: 
...
```ts
  ...
  18 | 
  19 | await page.fill('input[name="middleName"]', 'NewMiddleName'); // Clear and enter new middle name
  20 | 
  21 | await page.click('button[type="submit"]'); // Click "Save"
  22 | await page.reload(); // Refresh the page
  23 | const updatedName = await page.inputValue('input[name="firstName"]');
> 24 | expect(updatedName).toBe('NewName'); // Verify the updated name is saved
     |                     ^ Error: expect(received).toBe(expected) // Object.is equality
  25 | const updatedMiddleName = await page.inputValue('input[name="middleName"]');
  26 | expect(updatedMiddleName).toBe('NewMiddleName'); // Verify the updated middle name is saved
  27 | });
```