...
Call log:
  - waiting for locator('.oxd-select-option').filter({ hasText: 'Full-Time Permanent' })
...
```ts
  ...
  63 | 
  64 | await secondAddButton.isVisible();
> 65 | await page.locator('.oxd-select-option', { hasText: 'Full-Time Permanent' }).click();
     |                                                                              ^ Error: locator.click: Target page, context or browser has been closed
  66 | 
  67 | const fileChooserPromise2 = page.waitForEvent('filechooser');
  68 |   
  69 |   await page.locator('.oxd-file-button', { hasText: 'Browse' }).click();
  70 |   
  71 |   const fileChooser2 = await fileChooserPromise2;
  72 |   await fileChooser2.setFiles('/Users/rameshbalasubramanian/PlaywrightAutomation/Samplefileupload.rtf');
  73 | 
  74 | await page.locator('form button[type="submit"]', { hasText: 'Save' }).last().click();
  75 | await secondAddButton.getByText('Successfully Uploaded').waitFor(); // Wait for upload confirmation toast
  76 | 
  77 | });
```