...
```ts
  ...
  24 | 
> 25 | await page.getByText('other').click();
     |                               ^ Error: locator.click: Target page, context or browser has been closed
  26 | 
  27 | await page.fill('input[name="relationship"]', 'SpecifiedRelationship'); // Specify the relationship
  28 | 
  29 | await page.getByRole('textbox').nth(2).fill('test');
  30 | await page.click('button[type="submit"]'); // Click "Save"
  31 | });
```